...
var Tokenizer = exports.Tokenizer = function(input, document, tree) {
	...
	var emitToken = function emitToken(tok) { 
		...
		logger.log('tokenizer.token', tok);
		this.emit('token', tok);
	}.bind(this);
	...
	function consume_entity(buffer, allowed_char, from_attr) {
		...
		var c;
		if (chars === constants.EOF) return false;
	...
	function consume_numeric_entity(buffer, hex) {
		...
		var c = buffer.char();
		while(c !== constants.EOF && allowed.test(c)) {
			chars = chars + c;
			c = buffer.char();
		}
	...
	function markup_declaration_open_state(buffer) {
		...
		if (chars === '--') {
			...
			if (newchars === constants.EOF || chars === constants.EOF) {
