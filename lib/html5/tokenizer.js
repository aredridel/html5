...
HTML5.Tokenizer = t = function HTML5Tokenizer(input, ready) {
	...
	this.__defineSetter__('content_model', function(model) {
		HTML5.debug('tokenizer.content_model', HTML5.CONTENT_MODEL_FLAGS[model])
		content_model = model
	})
...
t.prototype.consume_entity = function(buffer, from_attr) {
	...
	if(chars.match(HTML5.SPACE_CHARACTERS) || chars == '<' || chars == '&') {
		...
		} else if(HTML5.DIGITS_R.test(chars[1])) {
			// Decimal entity
		...
		if(entityName) {
			...
			if(entityName[entityName.length - 1] != ';' && this.from_attribute && (HTML5.ASCII_LETTERS.test(chars.substr(entityName.length, 1) || HTML5.DIGITS.test(chars.substr(entityName.length, 1))))) {
				buffer.unget(chars);
				char = '&';
			} else {
...
t.prototype.data_state = function(buffer) {
	var c = buffer.shift(1);
	HTML5.debug('tokenizer.data_state', c)
	...
	if(c == "&" && (this.content_model == Models.PCDATA || this.content_model == Models.RCDATA) && !this.escapeFlag) {
		...
		HTML5.debug('tokenizer.data_state', 'reached EOF');
		this.emitToken(HTML5.EOF);
		this.commit();
		return false;
	} else {
		var o = buffer.matchUntil("[&<>-]")
		HTML5.debug('tokenizer.data_state', 'else: ' + c + o)
...
t.prototype.close_tag_open_state = function(buffer) {
	if(this.content_model == Models.RCDATA || this.content_model == Models.CDATA) {
		...
		if(this.current_token
			...
			&& (chars.length > this.current_token.name.length ? new RegExp('[' + HTML5.SPACE_CHARACTERS_IN + '></]').test(chars.substr(-1)) : true)
		) {
			this.content_model = Models.PCDATA;
		} else {
			this.emitToken({type: 'Characters', data: '</'});
...
t.prototype.attribute_value_unquoted_state = function(buffer) {
	...
	if(HTML5.SPACE_CHARACTERS_R.test(data)) {
		...
		HTML5.debug('tokenizer.attribute_value_unquoted_state', o)
		this.current_token.data.last()[1] += data + o
	}
	return true;
}
...
t.prototype.before_doctype_name_state = function(buffer) {
	var data = buffer.shift(1);
	HTML5.debug('tokenizer.before_doctype_name_state', data)
	if(HTML5.SPACE_CHARACTERS_R.test(data)) {
	} else if(data == '>') {
