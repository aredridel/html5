...
var enities  = require('html5-entities');
...
var ENTITY_KEYS = Object.keys(enities);
...
var Tokenizer = exports.Tokenizer = function(input, tree) {
	...
	function data_state(buffer) {
		...
		if (data === Buffer.EOF) {
			...
			emitToken({type: 'SpaceCharacters', data: data + buffer.matchWhile(constants.SPACE_CHARACTERS)});
			buffer.commit();
		} else {
	...
	function entity_data_state(buffer) {
	...
	function consume_entity(buffer, allowed_char, from_attr) {
		...
		var c;
		if (chars === Buffer.EOF) return false;
		if (chars.match(constants.SPACE_CHARACTERS) || chars == '<' || chars == '&'
			|| (allowed_char && allowed_char == chars)) {
			buffer.unget(chars);
		} else if (chars[0] == '#') { // Maybe a numeric entity
			c = buffer.shift(2);
			if (c === Buffer.EOF) {
				parseError("expected-numeric-entity-but-got-eof");
				buffer.unget(chars);
				return false;
			}
			chars += c;
			if (chars[1] && chars[1].toLowerCase() == 'x' && constants.HEX_DIGITS_R.test(chars[2])) {
				// Hex entity
				buffer.unget(chars[2]);
				char = consume_numeric_entity(buffer, true);
			} else if (chars[1] && constants.DIGITS_R.test(chars[1])) {
				// Decimal entity
				...
				// Not numeric
				buffer.unget(chars);
				parseError("expected-numeric-entity");
			}
		} else {
			...
			if (entityName) {
				if (entityName[entityName.length - 1] != ';') {
					parseError("named-entity-without-semicolon");
				}
				if (entityName[entityName.length - 1] != ';' && from_attr && (constants.ASCII_LETTERS_R.test(chars.substr(entityName.length, 1)) || constants.DIGITS_R.test(chars.substr(entityName.length, 1)) || chars.substr(entityName.length, 1) == '=')) {
					buffer.unget(chars);
					char = '&';
				} else {
					buffer.unget(chars.slice(entityName.length));
					char = enities[entityName];
				}
			} else {
				parseError("expected-named-entity");
				buffer.unget(chars);
			}
		}
	...
	function consume_numeric_entity(buffer, hex) {
		var allowed, radix;
		if (hex) {
			allowed = constants.HEX_DIGITS_R;
			radix = 16;
		} else {
			allowed = constants.DIGITS_R;
			radix = 10;
		}
		...
		var c = buffer.char();
		while(c !== Buffer.EOF && allowed.test(c)) {
			chars = chars + c;
			c = buffer.char();
		}
		...
		if (replacement) {
			parseError("invalid-numeric-entity-replaced");
		...
		/*if (charAsInt <= 0x10FFFF && !(charAsInt >= 0xD800 && charAsInt <= 0xDFFF)) {
			...
			parseError("cant-convert-numeric-entity");
		} */

		if (c !== ';') {
			parseError("numeric-entity-without-semicolon");
			buffer.unget(c);
		} 
	...
	function tag_open_state(buffer) {
		var data = buffer.char();
		if (data === Buffer.EOF) {
			parseError("bare-less-than-sign-at-eof");
			emitToken({type: 'Characters', data: '<'});
			buffer.unget(data);
			newState(data_state);
		} else if (isAlpha(data)) {
			...
			// do we really care?
			parseError("expected-tag-name-but-got-right-bracket");
			emitToken({type: 'Characters', data: "<>"});
			newState(data_state);
		} else if (data == '?') {
			// XXX In theory it could be something besides a tag name. But
			// do we really care?
			parseError("expected-tag-name-but-got-question-mark");
			buffer.unget(data);
	...
	function close_tag_open_state(buffer) {
		var data = buffer.char();
		if (data === Buffer.EOF) {
			parseError("expected-closing-tag-but-got-eof");
			emitToken({type: 'Characters', data: '</'});
			buffer.unget(data);
			newState(data_state);
		} else if (isAlpha(data)) {
			...
			parseError("expected-closing-tag-but-got-right-bracket");
			newState(data_state);
		} else {
			parseError("expected-closing-tag-but-got-char", {data: data}); // param 1 is datavars:
			buffer.unget(data);
			newState(bogus_comment_state);
		}
		return true;
	...
	function tag_name_state(buffer) {
		var data = buffer.char();
		if (data === Buffer.EOF) {
			parseError('eof-in-tag-name');
			buffer.unget(data);
			newState(data_state);
		} else if (isWhiteSpace(data)) {
			newState(before_attribute_name_state);
		} else if (isAlpha(data)) {
			current_token.name += data + buffer.matchWhile(constants.ASCII_LETTERS);
		} else if (data == '>') {
			emitCurrentToken();
		} else if (data == '/') {
			newState(self_closing_tag_state);
	...
	function before_attribute_name_state(buffer) {
		var data = buffer.shift(1);
		if (data === Buffer.EOF) {
			parseError("expected-attribute-name-but-got-eof");
			buffer.unget(data);
			newState(data_state);
		} else if (isWhiteSpace(data)) {
			buffer.matchWhile(constants.SPACE_CHARACTERS);
		} else if (isAlpha(data)) {
			...
			parseError("invalid-character-in-attribute-name");
			current_token.data.push({nodeName: data, nodeValue: ""});
			newState(attribute_name_state);
		} else if (data == '\u0000') {
			parseError("invalid-codepoint");
	...
	function attribute_name_state(buffer) {
		...
		var emitToken = false;
		if (data === Buffer.EOF) {
			parseError("eof-in-attribute-name");
			buffer.unget(data);
			newState(data_state);
			emitToken = true;
		} else if (data == '=') {
			...
			current_token.data.last().nodeName += data + buffer.matchWhile(constants.ASCII_LETTERS);
	...
	function after_attribute_name_state(buffer) {
		var data = buffer.shift(1);
		if (data === Buffer.EOF) {
			parseError("expected-end-of-tag-but-got-eof");
			buffer.unget(data);
			newState(data_state);
		} else if (isWhiteSpace(data)) {
			buffer.matchWhile(constants.SPACE_CHARACTERS);
	...
	function before_attribute_value_state(buffer) {
		var data = buffer.shift(1);
		if (data === Buffer.EOF) {
			parseError("expected-attribute-value-but-got-eof");
			buffer.unget(data);
			newState(data_state);
		} else if (isWhiteSpace(data)) {
			buffer.matchWhile(constants.SPACE_CHARACTERS);
			...
			parseError("expected-attribute-value-but-got-right-bracket");
			emitCurrentToken();
		} else if (data == '=' || data == '<' || data == '`') {
	...
	function attribute_value_double_quoted_state(buffer) {
		var data = buffer.shift(1);
		if (data === Buffer.EOF) {
			parseError("eof-in-attribute-value-double-quote");
			buffer.unget(data);
			newState(data_state);
		} else if (data == '"') {
	...
	function attribute_value_single_quoted_state(buffer) {
		var data = buffer.shift(1);
		if (data === Buffer.EOF) {
			parseError("eof-in-attribute-value-single-quote");
			buffer.unget(data);
			newState(data_state);
		} else if (data == "'") {
			newState(after_attribute_value_state);
	...
	function attribute_value_unquoted_state(buffer) {
		var data = buffer.shift(1);
		if (data === Buffer.EOF) {
			parseError("eof-after-attribute-value");
			buffer.unget(data);
			newState(data_state);
		} else if (isWhiteSpace(data)) {
			newState(before_attribute_name_state);
			...
			var o = buffer.matchUntil("\u0000|["+ constants.SPACE_CHARACTERS_IN + '&<>"\'=`' +"]");
			if (o === Buffer.EOF) {
				parseError("eof-in-attribute-value-no-quotes");
				emitCurrentToken();
			}
	...
	function self_closing_tag_state(buffer) {
		...
		if (c === Buffer.EOF) {
			parseError("unexpected-eof-after-solidus-in-tag");
			buffer.unget(c);
			newState(data_state);
		} else if (c == '>') {
			...
			parseError("unexpected-character-after-solidus-in-tag");
			buffer.unget(c);
			newState(before_attribute_name_state);
		}
		return true;
	...
	function markup_declaration_open_state(buffer) {
		...
		if (chars === '--') {
			...
			if (newchars === Buffer.EOF || chars === Buffer.EOF) {
				parseError("expected-dashes-or-doctype");
				newState(bogus_comment_state);
				buffer.unget(chars);
				return true;
			}
	...
	function comment_end_dash_state(buffer) {
		var data = buffer.char();
		if (data === Buffer.EOF) {
			parseError("eof-in-comment-end-dash");
			emitToken(current_token);
			buffer.unget(data);
			newState(data_state);
		} else if (data == '-') {
	...
	function comment_end_state(buffer) {
		var data = buffer.shift(1);
		if (data === Buffer.EOF) {
			parseError("eof-in-comment-double-dash");
			emitToken(current_token);
			buffer.unget(data);
			newState(data_state);
		} else if (data == '>') {
			...
			parseError("unexpected-bang-after-double-dash-in-comment");
			newState(comment_end_bang_state);
		} else if (data == '-') {
			parseError("unexpected-dash-after-double-dash-in-comment");
			current_token.data += data;
		} else if (data == '\u0000') {
			parseError("invalid-codepoint");
			current_token.data += "--\uFFFD";
			...
			// XXX
			parseError("unexpected-char-in-comment");
			current_token.data += '--' + data;
			newState(comment_state);
		}
		return true;
	...
	function comment_end_bang_state(buffer) {
		var data = buffer.shift(1);
		if (data === Buffer.EOF) {
			parseError("eof-in-comment-end-bang-state");
			emitToken(current_token);
			buffer.unget(data);
			newState(data_state);
		} else if (data == '>') {
	...
	function doctype_state(buffer) {
		var data = buffer.shift(1);
		if (data === Buffer.EOF) {
			parseError("expected-doctype-name-but-got-eof");
			...
			parseError("need-space-after-doctype");
			buffer.unget(data);
			newState(before_doctype_name_state);
		}
		return true;
	...
	function before_doctype_name_state(buffer) {
		...
		if (data === Buffer.EOF) {
			...
			parseError("expected-doctype-name-but-got-right-bracket");
			current_token.correct = false;
			newState(data_state);
			emitCurrentToken();
		} else {
	...
	function doctype_name_state(buffer) {
		...
		if (data === Buffer.EOF) {
			current_token.correct = false;
			buffer.unget(data);
			parseError("eof-in-doctype-name");
			newState(data_state);
			emitCurrentToken();
		} else if (isWhiteSpace(data)) {
	...
	function after_doctype_name_state(buffer) {
		...
		if (data === Buffer.EOF) {
			...
			if (data === Buffer.EOF) {
				...
				parseError("expected-space-or-right-bracket-in-doctype", {data: data});
	...
	function before_doctype_public_identifier_state(buffer) {
		...
		if (data === Buffer.EOF) {
			...
			parseError("unexpected-end-of-doctype");
			current_token.correct = false;
			newState(data_state);
			emitCurrentToken();
		} else {
	...
	function parseError(message, context) {
		...
		logger.log('tokenizer.parseError', message, context);
	}
	...
	function normalize_token(token) {
		if (token.type == 'StartTag') {
			...
			if (token.self_closing && constants.VOID_ELEMENTS.indexOf(token.name) == -1) {
				parseError('non-void-element-with-trailing-solidus', {name: token.name});
			}
		} else if (token.type == 'EndTag') {
			token.name = token.name.toLowerCase();
			if (token.self_closing) {
				parseError('self-closing-flag-on-end-tag');
			}
			if (token.data.length !== 0) {
				parseError('attributes-in-end-tag');
			}
		}
	...
	function newState(newstate) {
		logger.log('tokenizer.state=', newstate.name);
		state = newstate;
		buffer.commit();
	}
	...
	source.addListener('data', function(data) {
		...
		try {
			...
			if (e != Buffer.DRAIN) {
				...
				logger.log('tokenizer.drain', 'Drain');
				buffer.undo();
			}
		}
	});
