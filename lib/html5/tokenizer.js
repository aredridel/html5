...
HTML5.Tokenizer = t = function HTML5Tokenizer(input, ready) {
	...
	this.__defineSetter__('state', function(newstate) {
		if(!this[newstate]) require('sys').debug(newstate + " is not defined");
		state = newstate;
		buffer.commit();
	});
...
t.prototype.emitToken = function(tok) { 
	HTML5.debug('tokenizer', 'emitToken');
	this.emit('token', tok);
}
...
t.prototype.consume_entity = function(buffer, from_attr) {
	...
	if(chars.match(HTML5.SPACE_CHARACTERS) || chars == '<' || chars == '&') {
		...
		if(chars[1].toLowerCase() == 'x' && HTML5.HEX_DIGITS.test(chars[2])) {
			// Hex entity
			buffer.unget(chars[2]);
			char = this.consume_number_entity(buffer, true);
		} else if(HTML5.DIGITS.test(chars[1])) {
			// Decimal entity
			buffer.unget(chars.slice(1));
			char = this.consume_number_entity(buffer, false);
		} else {
			// Not numeric
			buffer.unget(chars);
...
t.prototype.consume_numeric_entity = function(buffer, hex) {
	...
	if(hex) {
		var allowed = HTML5.HEX_DIGITS;
		var radix = 16;
	} else {
		var allowed = HTML5.DIGITS;
		var radix = 10;
	}
...
t.prototype.process_entity_in_attribute = function(buffer) {
	...
	if(entity) {
		this.current_token.data[this.current_token.data.length - 1][1] += entity;
	} else {
		this.current_token.data[this.current_token.data.length - 1][1] += '&';
	}
}
...
t.prototype.data_state = function(buffer) {
	HTML5.debug('tokenizer', 'data_state');
	var c = buffer.shift(1);
...
t.prototype.comment_end_dash_state = function(buffer) {
	HTML5.debug('tokenizer', 'comment_end_dash_state');
	var data = buffer.shift(1);
	if(data == '-') {
		this.state = 'comment_end_state';
	} else if (data == 'EOF') {
...
t.prototype.doctype_name_state = function(buffer) {
	...
	if(HTML5.SPACE_CHARACTERS_R.test(data)) {
		...
		if(token == 'public') {
			...
			buffer.unget(data);
			this.parse_error("expected-space-or-right-bracket-in-doctype");
