	...
	function consume_entity(buffer, from_attr) {
		...
		if (chars.match(HTML5.SPACE_CHARACTERS) || chars == '<' || chars == '&') {
			...
			if (entityName) {
				...
				if (entityName[entityName.length - 1] != ';' && from_attr && (HTML5.ASCII_LETTERS_R.test(chars.substr(entityName.length, 1) || HTML5.DIGITS.test(chars.substr(entityName.length, 1))))) {
					buffer.unget(chars);
					char = '&';
				} else {
