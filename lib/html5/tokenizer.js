...
t.prototype.emitToken = function(tok) { 
	...
	if (this.content_model == Models.SCRIPT_CDATA && (tok.type == 'Characters' || tok.type == 'SpaceCharacters') && !this.buffer.eof) {
...
var replaceEntityNumbers = function(c) {
	switch(c) {
...
t.prototype.consume_numeric_entity = function(buffer, hex) {
	...
	if(replacement = replaceEntityNumbers(charAsInt)) {
		this.parse_error("invalid-numeric-entity-replaced", {old: charAsInt, new: replacement});
...
/** @todo This probably needs to be multiple states. Getting pretty hairy, and
 * I think the spec has broken it up.
 */
t.prototype.data_state = function(buffer) {
	...
	if(c === HTML5.EOF) {
		...
		this.commit();
		return false;
	} else if(c === '\0' && (this.content_model == Models.SCRIPT_CDATA || this.content_model == Models.PLAINTEXT || this.content_model == Models.RAWTEXT || this.content_model == Models.RCDATA)) {
...
t.prototype.tag_open_state = function(buffer) {
	...
	if(this.content_model == Models.PCDATA) {
		if (data === HTML5.EOF) {
			this.parse_error("bare-less-than-sign-at-eof");
			this.emitToken({type: 'Characters', data: '<'});
			this.state = 'data_state';
...
t.prototype.tag_name_state = function(buffer) {
	...
	if(data === HTML5.EOF) {
		...
		if(c !== HTML5.EOF) {
			this.current_token.name += data + c
		} else {
			this.current_token_name += data
			buffer.unget(c)
			this.state = 'data_state'
		}
	} else if(data == '>') {
...
t.prototype.parse_error = function(message, context) {
...
t.prototype.emit_current_token = function() {
	...
	if (this.current_token.name == "script" && tok.type == 'EndTag' && this.script_buffer) {
