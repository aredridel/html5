...
b.prototype.insert_comment = function(data, parent) {
	...
	HTML5.debug('treebuilder', 'open elements at insert_comment are: ' + require('sys').inspect(this.open_elements));
...
b.prototype.insert_text = function(data, parent) {
	if(!parent) parent = this.open_elements.last();
	if(!this.insert_from_table || (this.insert_from_table && TABLE_INSERT_MODE_ELEMENTS.indexOf(this.open_elements.last().tagName) == -1)) {
...
b.prototype.elementInScope = function(name, tableVariant) {
	if(this.open_elements.last() && this.open_elements.last().tagName == name) return true;
