...
b.prototype.insert_element = function(name, attributes, namespace) {
	...
	HTML5.debug("inserting " +  name);
	return element;
}

b.prototype.insert_text = function(data, parent) {
	if(!parent) parent = this.open_elements[this.open_elements.length - 1];
HTML5.debug("inserting '" + data + "' into " + parent.tagName);
	if(!this.insert_from_table || (this.insert_from_table && TABLE_INSERT_MODE_ELEMENTS.indexOf(this.open_elements[this.open_elements.length - 1].tagName) == -1)) {
...
b.prototype.elementInScope = function(name, tableVariant) {
	if(this.open_elements[this.open_elements.length - 1] && this.open_elements[this.open_elements.length - 1].tagName == name) return true;
...
b.prototype.generateImpliedEndTags = function (exclude) {
	...
	var name = this.open_elements[this.open_elements.length - 1].tagName;
...
b.prototype.reconstructActiveFormattingElements = function() {
	...
	HTML5.debug("reconstructActiveFormattingElements with "+sys.inspect(this.activeFormattingElements));
