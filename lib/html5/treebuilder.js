...
HTML5.TreeBuilder = b = function TreeBuilder(DOM) {
	this.open_elements = [];
...
b.prototype.createElement = function (name, attributes, namespace) {
	...
	if(attributes) {
		if(attributes instanceof this.DOM.NamedNodeMap) {
			for(var i = 0; i < attributes.length; i++) {
				el.setAttribute(attributes.item(i).nodeName, attributes.item(i).nodeValue);
			}
		} else {
			for(var i = 0; i < attributes.length; i++) {
...
b.prototype.insert_doctype = function (name, publicId, systemId) {
	...
	var doctype = new this.DOM.DocumentType(this.document, name);
...
b.prototype.elementInScope = function(name, tableVariant) {
	...
	if(this.open_elements.last().tagName.toLowerCase() == name) return true
	for(var i = this.open_elements.length - 1; i >= 0; i--) {
		if(this.open_elements[i].tagName.toLowerCase() == name) return true
