...
function text(data, target) {
	var m;
	if(m = new RegExp("^[" + constants.SPACE_CHARACTERS + "]+").exec(data)) {
...
var TreeWalker = exports.TreeWalker = function(document, dest) {
...
function walk(node, dest) {
	...
	case node.ELEMENT_NODE:
		if(constants.VOID_ELEMENTS.indexOf(node.tagName.toLowerCase()) != -1) {
