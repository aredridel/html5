...
function empty_tag(node) {
	...
	return {type: 'EmptyTag', name: node.tagName, data: normalize_attrs(node.attributes), namespace: node.namespace};
}

function start_tag(node) {
	return {type: 'StartTag', name: node.tagName, data: normalize_attrs(node.attributes), namespace: node.namespace};
}
