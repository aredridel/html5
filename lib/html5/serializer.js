...
exports.serialize = function(src, target, override) {
	var options;
	if(!override) {
	...
	var w = new TreeWalker(src, function(tok) {
		if(tok.type == "Doctype") {
			doctype = "<!DOCTYPE " + tok.name;
			if (tok.publicId)
				doctype += ' PUBLIC "' + tok.publicId + '"';
			else if (tok.systemId)
				doctype += " SYSTEM";
			if (tok.systemId) {
				if (tok.systemId.search('"') >= 0) {
					if (tok.systemId.search("'") >= 0)
				...
				doctype += " " + quote_char + tok.systemId + quote_char;
			}
			doctype += ">";
			...
			if(constants.RCDATA_ELEMENTS.indexOf(tok.name.toLowerCase()) != -1 && !escape_rcdata) {
				in_cdata = true;
			} else if (in_cdata) {
			...
			for(var ki in attrs) {
				...
				if(!options.minimize_boolean_attributes || ((constants.BOOLEAN_ATTRIBUTES[tok.name] || []).indexOf(ki) == -1 && (constants.BOOLEAN_ATTRIBUTES["_global"].indexOf(ki) == -1))) {
					attributes += "=";
					if(options.quote_attr_values || v.length == 0) {
						quote_attr = true;
					} else {
						quote_attr = new RegExp("[" + constants.SPACE_CHARACTERS_IN + "<=>'\"" + "]").test(v)
					}
			...
			if(constants.VOID_ELEMENTS.indexOf(tok.name.toLowerCase()) != -1 && options.use_trailing_solidus) {
			...
			if(constants.RCDATA_ELEMENTS.indexOf(tok.name.toLowerCase()) != -1) {
				in_cdata = false;
			} else if(in_cdata) {
				serialize_error("Unexpected child element of a CDATA element");
			}
