...
function formatMessage(format, args) {
	return format.replace(new RegExp('{[0-9a-z-]+}', 'g'), function(match) {
...
var Parser = exports.Parser = function(options) {
	...
	this.newPhase = function(name) {
		...
		logger.log('parser.newPhase', name);
		this.phaseName = name;
	};
	...
	phases.base = {
		...
		inScope: function(name, scopingElements) {
			if (!scopingElements) scopingElements = constants.SCOPING_ELEMENTS;
	...
	phases.inCaption.ignoreEndTagCaption = function() {
		return !this.inScope('caption', constants.TABLE_SCOPING_ELEMENTS);
	};
	...
	phases.inCell.startTagTableOther = function(name, attributes, self_closing) {
		if (this.inScope('td', constants.TABLE_SCOPING_ELEMENTS) || this.inScope('th', constants.TABLE_SCOPING_ELEMENTS)) {
			this.closeCell();
			phase.processStartTag(name, attributes, self_closing);
		} else {
			// inner_html case
	...
	phases.inCell.endTagTableCell = function(name) {
		if (this.inScope(name, constants.TABLE_SCOPING_ELEMENTS)) {
	...
	phases.inCell.closeCell = function() {
		if (this.inScope('td', constants.TABLE_SCOPING_ELEMENTS)) {
			this.endTagTableCell('td');
		} else if (this.inScope('th', constants.TABLE_SCOPING_ELEMENTS)) {
	...
	phases.inForeignContent.processCharacters = function(characters) {
		...
		if (parser.framesetOk && new RegExp('[^' + '\uFFFD' + constants.SPACE_CHARACTERS_IN + ']').test(characters))
			parser.framesetOk = false;
	...
	phases.inHead.processCharacters = function(data) {
		var name = tree.open_elements.last().tagName.toLowerCase();
		logger.log('parser.inHead.processCharacters', data);
	...
	phases.inTable.startTagCaption = function(name, attributes) {
		...
		tree.activeFormattingElements.push(constants.Marker);
		tree.insert_element(name, attributes);
	...
	phases.inTableText.flushCharacters = function() {
		...
		if (new RegExp('[^' + constants.SPACE_CHARACTERS_IN + ']').test(data))
	...
	phases.inTableBody.startTagTableOther = function(name, attributes) {
		...
		if (this.inScope('tbody', constants.TABLE_SCOPING_ELEMENTS) ||  this.inScope('thead', constants.TABLE_SCOPING_ELEMENTS) || this.inScope('tfoot', constants.TABLE_SCOPING_ELEMENTS)) {
			this.clearStackToTableBodyContext();
	...
	phases.inSelect.endTagSelect = function(name) {
		if (this.inScope('select', constants.TABLE_SCOPING_ELEMENTS)) {
	...
	phases.inSelect.startTagInput = function(name, attributes) {
		...
		if (this.inScope('select', constants.SELECT_SCOPING_ELEMENTS)) {
			this.endTagSelect('select');
			phase.processStartTag(name, attributes);
		}
	};
	...
	phases.inRow.ignoreEndTagTr = function() {
		return !this.inScope('tr', constants.TABLE_SCOPING_ELEMENTS);
	};
	...
	phases.afterAfterFrameset.processEndTag = function(name, attributes) {
		parser.parse_error('expected-eof-but-got-end-tag', {name: name});
	};
	...
	phases.inBody.processCharacters = function(data) {
		...
		tree.reconstructActiveFormattingElements();
		tree.insert_text(data);
		if (parser.framesetOk && new RegExp('[^' + constants.SPACE_CHARACTERS_IN + ']').test(data))
			parser.framesetOk = false;
	};
	...
	phases.inBody.startTagCloseP = function(name, attributes) {
		if (this.inScope('p', constants.BUTTON_SCOPING_ELEMENTS)) this.endTagP('p');
		tree.insert_element(name, attributes);
	};
	...
	phases.inBody.startTagListItem = function(name, attributes) {
		...
		for(var i = els.length - 1; i >= 0; i--) {
			...
			// formatting elements
			if (((node.namespace || 'html') in constants.SPECIAL_ELEMENTS &&
					constants.SPECIAL_ELEMENTS[node.namespace || 'html'].indexOf(node.tagName.toLowerCase()) != -1)/* ||
				((node.namespace || 'html') in constants.SCOPING_ELEMENTS
					&& constants.SCOPING_ELEMENTS[node.namespace || 'html'].indexOf(node.tagName.toLowerCase()) != -1)*/ &&
	...
	phases.inBody.startTagHeading = function(name, attributes) {
		if (this.inScope('p', constants.BUTTON_SCOPING_ELEMENTS)) this.endTagP('p');
		if (constants.HEADING_ELEMENTS.indexOf(tree.open_elements.last().tagName.toLowerCase()) != -1) {
			parser.parse_error('unexpected-start-tag', {name: name});
			tree.pop_element();
		}
	...
	phases.inBody.startTagXmp = function(name, attributes) {
		if (this.inScope('p', constants.BUTTON_SCOPING_ELEMENTS)) this.processEndTag('p');
		tree.reconstructActiveFormattingElements();
		parser.parseRCDataRawText("RAWTEXT", name, attributes);
		parser.framesetOk = false;
	};
	...
	phases.inBody.startTagTable = function(name, attributes) {
		if (parser.compatMode !== "quirks")
			if (this.inScope('p', constants.BUTTON_SCOPING_ELEMENTS))
	...
	phases.inBody.endTagOther = function endTagOther(name) {
		...
		for(var eli = nodes.length - 1; eli > 0; eli--) {
			...
			if (nodes[eli].tagName.toLowerCase() == name) {
				...
				if (((currentNode.namespace || 'html') in constants.SPECIAL_ELEMENTS &&
						constants.SPECIAL_ELEMENTS[currentNode.namespace || 'html'].indexOf(currentNode.tagName.toLowerCase()) != -1) ||
					((currentNode.namespace || 'html') in constants.SCOPING_ELEMENTS &&
						constants.SCOPING_ELEMENTS[currentNode.namespace || 'html'].indexOf(currentNode.tagName.toLowerCase()) != -1)
				) {
					parser.parse_error('unexpected-end-tag', {name: name});
					break;
				}
	...
	phases.inBody.endTagListItem = function(name) {
		if (!this.inScope(name, constants.LIST_SCOPING_ELEMENTS)) {
			parser.parse_error('unexpected-end-tag', {name: name});
		} else {
			tree.generateImpliedEndTags(name);
			if (tree.open_elements.last().tagName.toLowerCase() != name)
	...
	phases.inBody.endTagHeading = function(name) {
		...
		for(i in constants.HEADING_ELEMENTS) {
			var el = constants.HEADING_ELEMENTS[i];
			if (this.inScope(el)) {
				error = false;
				break;
			}
		...
		tree.remove_open_elements_until(function(e) {
			return constants.HEADING_ELEMENTS.indexOf(e.tagName.toLowerCase()) != -1;
		});
	};
	...
	phases.inBody.endTagFormatting = function(name, attributes) {
		...
		while (outerLoopCounter++ < 8) {
			...
			for (var i = 0; i < len; i++) {
				element = els[i];
				if (((element.namespace || 'html') in constants.SPECIAL_ELEMENTS &&
						constants.SPECIAL_ELEMENTS[element.namespace || 'html'].indexOf(element.tagName.toLowerCase()) != -1) ||
					((element.namespace || 'html') in constants.SCOPING_ELEMENTS &&
						constants.SCOPING_ELEMENTS[element.namespace || 'html'].indexOf(element.tagName.toLowerCase()) != -1)
				) {
	...
	var tree = this.tree = new TreeBuilder(this.document);
};
...
Parser.prototype.parse_error = function(code, args, isWarning) {
	// FIXME: this.errors.push([this.tokenizer.position, code, data]);

	if (code === undefined) {
		throw ('code is undefied');
	}
	var message = formatMessage(messages[code], args);
	this.errors.push([message ,isWarning]);
...
Parser.prototype.reset_insertion_mode = function() {
	...
	for(var i = this.tree.open_elements.length - 1; i >= 0; i--) {
		...
		if (constants.TAGMODES[node_name]) {
			this.newPhase(constants.TAGMODES[node_name]);
		} else if (last) {
...
Parser.prototype.is_html_integration_point = function(element) {
	if (element.namespace in constants.HTML_INTEGRATION_POINT_ELEMENTS &&
		constants.HTML_INTEGRATION_POINT_ELEMENTS[element.namespace].indexOf(element.tagName.toLowerCase()) > -1) {
...
Parser.prototype.is_mathml_text_integration_point = function(element) {
	if (element.namespace in constants.MATHML_TEXT_INTEGRATION_POINT_ELEMENTS &&
		constants.MATHML_TEXT_INTEGRATION_POINT_ELEMENTS[element.namespace].indexOf(element.tagName.toLowerCase()) > -1) {
		return true;
	}
	return false;
};
