...
var Parser = HTML5.Parser = function HTML5Parser(options) {
	...
	phases.inForeignContent.startTagOther = function(name, attributes, self_closing) {
		...
		if (!currentNode.namespace
			|| (parser.is_mathml_text_integration_point(currentNode) && !(name in {'mglyph':1, 'malignmark':1}))
			|| (name == 'svg' && currentNode.tagName.toLowerCase() == 'annotation-xml' && currentNode.namespace == 'math')
			|| parser.is_html_integration_point(currentNode)) {
			secondary_phase.processStartTag(name, attributes);
			if (phase == 'inForeignContent') {
				if (tree.open_elements.some(function(e) { return e.namespace; })) {
					phase = secondary_phase;
				}
			}
		} else if (['b', 'big', 'blockquote', 'body', 'br', 'center', 'code', 'dd', 'div', 'dl', 'dt', 'em', 'embed', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'head', 'hr', 'i', 'img', 'li', 'listing', 'menu', 'meta', 'nobr', 'ol', 'p', 'pre', 'ruby', 's', 'small', 'span', 'strong', 'strike', 'sub', 'sup', 'table', 'tt', 'u', 'ul', 'var'].indexOf(name) != -1
	...
	phases.inForeignContent.processCharacters = function(characters) {
		if (framesetOk && new RegExp('^' + HTML5.HTML5.SPACE_CHARACTERS + '*$').test(characters))
			framesetOk = false;
	...
	phases.inBody.processCharacters = function(data) {
		...
		tree.reconstructActiveFormattingElements();
		tree.insert_text(data);
		if (framesetOk && new RegExp('^' + HTML5.HTML5.SPACE_CHARACTERS + '*$').test(data))
			framesetOk = false;
	};
	...
	phases.inBody.startTagListItem = function(name, attributes) {
		...
		for(var i = els.length - 1; i >= 0; i--) {
			...
			// formatting elements
			if (HTML5.SPECIAL_ELEMENTS.concat(HTML5.SCOPING_ELEMENTS).indexOf(node.tagName.toLowerCase()) != -1 && ['address', 'div', 'p'].indexOf(node.tagName.toLowerCase()) == -1) break;
		}
		if (this.inScope('p', HTML5.BUTTON_SCOPING_ELEMENTS)) this.endTagP('p');
	...
	phases.inBody.startTagIFrame = function(name, attributes) {
		framesetOk = false;
		this.startTagCdata(name, attributes);
	};
...
Parser.prototype.is_html_integration_point = function(element) {
	if (element.namespace in HTML5.HTML_INTEGRATION_POINT_ELEMENTS &&
		...
		return element.tagName.toLowerCase() != 'annotation-xml' || (['text/html', 'application/xhtml+xml'].indexOf((element.getAttribute('encoding') || '').toLowerCase()));
	}
	return false;
};
