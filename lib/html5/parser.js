...
HTML5.Parser = Parser = function HTML5Parser(source, options) {
	...
	if(options) for(o in options) {
		HTML5.debug('parser.options', o, options[o])
		this[o] = options[o];
	}
...
Parser.prototype._parse = function(inner_html, encoding, container) {
	...
	if(inner_html) {
		HTML5.debug('parser.inner_html', "inner_html with container", container);
...
Parser.prototype.reset_insertion_mode = function() {
	...
	for(var i = this.tree.open_elements.length - 1; i >= 0; i--) {
		node_name = this.tree.open_elements[i].tagName;
		if(i == 0) {
			last = true ;
		...
		if(HTML5.TAGMODES[node_name]) {
			...
			this.newPhase(this.tree.head_pointer ? 'beforeHead' : 'afterHead');
		} else if(last) {
