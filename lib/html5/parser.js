...
var Parser = HTML5.Parser = function HTML5Parser(source, options) {
	...
	if(!this.DOM) {
		...
		this.DOM = browser.browserAugmentation(require('jsdom/level2/core').dom.level2.core) 
	}

	this.tree = new this.tree(this.DOM);

	var that = this;

	new HTML5.Tokenizer(source, function(tokenizer) {
		that.tokenizer = tokenizer;
		tokenizer.DOM = that.DOM;
		tokenizer.document = that.tree.document;
