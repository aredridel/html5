...
p.startTagTr = function(name, attributes) {
	this.clearStackToTableBodyContent();
...
p.startTagTableOther = function(name, attributes) {
	...
	if(this.inScope('tbody', true) ||  this.inScope('thead', true) || this.inScope('tfoot', true)) {
		...
		this.endTagTableRowGroup(this.open_elements.last().tagName);
		this.parser.phase.processStartTag(name, attributes);
	} else {
		// inner_html case
		this.parse_error
...
p.endTagTable = function(name) {
	if(this.inScope('tbody', true) || this.inScope('thead', true) || this.inScope('tfoot', true)) {
		this.clearStackToTableBodyContext();
		this.endTagTableRowGroup(this.tree.open_elements.last().tagName)
	} else {
		// inner_html case
		this.parse_error();
	}
...
p.clearStackToTableBodyContext = function() {
	while(name = this.tree.open_elements.last().tagName, name != 'tbody' && name != 'tfoot' && name != 'thead' && name != 'html') {
