...
p.prototype.startTagListItem = function(name, attributes) {
	...
	for(var i = els.length - 1; i >= 0; i--) {
		...
		// formatting elements
		if(HTML5.SPECIAL_ELEMENTS.concat(HTML5.SCOPING_ELEMENTS).indexOf(node.tagName) != -1 && (node.tagName != 'address' && node.tagName != 'div')) break;
	}
...
p.prototype.endTagAppletButtonMarqueeObject = function(name) {
	...
	if(this.inScope(name)) {
		this.remove_open_elements_until(name)
...
p.prototype.endTagP = function(name) {
	...
	if(!this.tree.open_elements.last().tagName == 'p')
...
p.prototype.endTagBody = function(name) {
	...
	if(this.tree.open_elements.last().tagName != 'body') {
		this.parse_error('expected-one-end-tag-but-got-another', {
			expectedName: 'body',
			gotName: this.tree.open_elements.last().tagName
		});
	}
...
HTML5.debug('parser.inBody.endTagBlock', HTML5.dumpTagStack(this.tree.open_elements))
	if(this.inScope(name)) this.tree.generateImpliedEndTags();
	if(!this.tree.open_elements.last().tagName == 'name') {
		this.parse_error('end-tag-too-early', {name: name});
	}
