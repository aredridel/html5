...
p.prototype.processSpaceCharactersDropNewline = function(data) {
	...
	if(data.length > 0 && data[0] == "\n" && ('pre' == this.tree.open_elements.last().tagName || 'textarea' == this.tree.open_elements.last().tagName) && !this.tree.open_elements.last().hasChildNodes()) {
		data = data.slice(1)
	}

	if(data.length > 0) {
		this.tree.reconstructActiveFormattingElements()
		this.tree.insert_test(data)
	}
}
...
p.prototype.startTagListItem = function(name, attributes) {
	...
	for(var i = els.length - 1; i >= 0; i--) {
		var node = els[i];
		if(stopName.indexOf(node.tagName) != -1) {
			var poppedNodes = [];
		...
		// formatting elements
		if(HTML5.SPECIAL_ELEMENTS.concat(HTML5.SCOPING_ELEMENTS).indexOf(node.name) != -1 && (node.name != 'address' && node.name != 'div')) break;
	}
...
p.prototype.startTagIsindex = function(name, attributes) {
	...
	HTML5.debug('parser.inBodyPhase.startTagIsindex', attributes)
...
p.prototype.endTagOther = function endTagOther(name) {
	HTML5.debug('parser.inBody.endTagOther', name)
	var nodes = this.tree.open_elements;
	for(var eli = nodes.length - 1; eli > 0; eli--) {
		var currentNode = nodes[eli];
		if(nodes[eli].tagName == name) {
			this.tree.generateImpliedEndTags();
			if(this.tree.open_elements.last().tagName != name) {
				this.parse_error('unexpected-end-tag', {name: name});
			}
			
			this.remove_open_elements_until(function(el) {
			...
			if(HTML5.SPECIAL_ELEMENTS.concat(HTML5.SCOPING_ELEMENTS).indexOf(nodes[eli].tagName) != -1) {
				this.parse_error('unexpected-end-tag', {name: name});   
				break;
			}
		}
...
p.prototype.endTagP = function(name) {
	...
	if(!this.tree.open_elements[this.tree.open_elements.length - 1].tagName == 'p')			this.parse_error('unexpected-end-tag', {name: 'p'});
	if(this.inScope('p')) {
...
p.prototype.endTagBody = function(name) {
	if(this.tree.open_elements[1].tagName != 'body') {
		// inner_html case
		this.parse_error('unexpected-end-tag', {name: 'body'});
		return;
	}

	if(this.tree.open_elements[this.tree.open_elements.length - 1].tagName != 'body') {
		this.parse_error('expected-one-end-tag-but-got-another', {
			expectedName: 'body',
			gotName: this.tree.open_elements[this.tree.open_elements.length - 1].tagName
		});
	}
...
p.prototype.endTagBlock = function(name) {
	...
	HTML5.debug('parser.endTagBlock', name)
...
p.prototype.endTagListItem = function(name) {
	...
	if(this.tree.open_elements[this.tree.open_elements.length - 1].tagName != name)
		this.parse_error('end-tag-too-early', {name: name});
	HTML5.debug('parser.endTagListItem', name)
	if(this.inScope(name)) this.remove_open_elements_until(name);
}
	...
	for(i in HTML5.HEADING_ELEMENTS) {
		...
		if(this.inScope(el)) {
			this.remove_open_elements_until(function(e) {
				return HTML5.HEADING_ELEMENTS.indexOf(e.tagName) != -1
			});
			break;
		}
	}
...
p.prototype.endTagFormatting = function(name) {
	while(true) {
		HTML5.debug('parser.endTagFormatting', 'Step 1')
		...
		if(!afeElement || (this.tree.open_elements.indexOf(afeElement) != -1
			&& !this.inScope(afeElement.tagName))) {
		...
		HTML5.debug('parser.endTagFormatting', 'Step 2')
		...
		for(var i in els) {
			var element = els[i];
			if(HTML5.SPECIAL_ELEMENTS.concat(HTML5.SCOPING_ELEMENTS).indexOf(element.tagName) != -1) {
				furthestBlock = element;
				break;
			}
		}
		
		HTML5.debug('parser.endTagFormatting', 'Step 3')
		if(!furthestBlock) {
			var element = this.remove_open_elements_until(function(el) {
		...
		HTML5.debug('parser.endTagFormatting', 'Step 4')
		...
		HTML5.debug('parser.endTagFormatting', 'Step 5')
		var bookmark = this.tree.activeFormattingElements.indexOf(afeElement);

		HTML5.debug('parser.endTagFormatting', 'Step 6')
		var lastNode;
		var node;
		lastNode = node = furthestBlock;

		while(true) {
			HTML5.debug('parser.endTagFormatting', 'Step 6.1')
			node = this.tree.open_elements[this.tree.open_elements.indexOf(node) - 1];
			HTML5.debug('parser.endTagFormatting', 'Step 6.2')
...
//HTML5.debug('parser.adoption', node, afeElement); break;
			
			HTML5.debug('parser.endTagFormatting', 'Step 6.3')
			if(node == afeElement) break;
			// FIXME -- should have broken

			HTML5.debug('parser.endTagFormatting', 'Step 6.4')
			...
			HTML5.debug('parser.endTagFormatting', 'Step 6.5')
			...
			HTML5.debug('parser.endTagFormatting', 'Step 6.6')
			if(lastNode.parent) lastNode.parent.removeChild(lastNode);
			node.appendChild(lastNode);
5
			HTML5.debug('parser.endTagFormatting', 'Step 6.7')
			lastNode = node

			HTML5.debug('parser.endTagFormatting', 'Step 6.8')
		}

		HTML5.debug('parser.endTagFormatting', 'Step 7a: FIXME')
		HTML5.debug('parser.endTagFormatting', 'Step 7b')
		if(lastNode.parent) lastNode.parent.removeChild(lastNode);
		commonAncestor.appendChild(lastNode);

		HTML5.debug('parser.endTagFormatting', 'Step 8')
		clone = afeElement.cloneNode();

		HTML5.debug('parser.endTagFormatting', 'Step 9')
		this.tree.reparentChildren(furthestBlock, clone);

		HTML5.debug('parser.endTagFormatting', 'Step 10')
		furthestBlock.appendChild(clone);

		HTML5.debug('parser.endTagFormatting', 'Step 11')
		...
		HTML5.debug('parser.endTagFormatting', 'Step 12')
		...
		HTML5.debug('parser.endTagFormatting', 'Step 13')
	}
}
