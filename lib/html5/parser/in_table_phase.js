...
p.prototype.endTagTable = function(name) {
	if(this.inScope(name, true)) {
		...
		if(this.tree.open_elements.last().nodeName != name) {
			this.parse_error("end-tag-too-early-named", {gotName: 'table', expectedName: this.tree.open_elements.last().tagName});
		}
...
p.prototype.clearStackToTableContext = function() {
	var name;
	while(name = this.tree.open_elements.last().tagName, (name != 'table' && name != 'html')) {
