...
p.startTagOption = function(name, attributes) {
	...
	if(this.tree.open_elements.last().tagName == 'option') this.tree.open_elements.pop();
	this.tree.insert_element(name, attributes);
}

p.startTagOptgroup = function(name, attributes) {
	if(this.tree.open_elements.last().tagName == 'option') this.tree.open_elements.pop();
	if(this.tree.open_elements.last().tagName == 'optgroup') this.tree.open_elements.pop();
	this.tree.insert_element(name, attributes);
}
	
p.endTagOption = function(name) {
	if(this.tree.open_elements.last().tagName == 'option') {
...
p.endTagOptgroup = function(name) {
	...
	if(this.tree.open_elements.last().tagName == 'option' && this.tree.open_elements[this.tree.open_elements.length - 2].tagName == 'optgroup') {
		this.tree.open_elements.pop();
	}

	// it also closes </optgroup>
	if(this.tree.open_elements.last().tagName == 'optgroup') {
		this.tree.open_elements.pop();
	} else {
		// But nothing else
...
p.endTagSelect = function(name) {
	if(this.inScope('select', true)) {
		this.remove_open_elements_until('select');
