...
p.prototype.processCharacters = function(data) {
	new inBody(parser, tree).processCharacters(data);
}
...
p.prototype.endTagCaption = function(name) {
	if(this.ignoreEndTagCaption()) {
		...
		if(this.tree.open_elements.last().tagName != 'caption') {
			this.parse_error('expected-one-end-tag-but-got-another',
                    		{gotName: "caption", expectedName: this.tree.open_elements.last().tagName});
		}

		this.remove_open_elements_until('caption');
