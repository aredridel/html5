...
function Tokenizer(tokenHandler) {
	...
	this._state = Tokenizer.DATA_STATE;
...
Tokenizer.prototype.tokenize = function(source) {
	...
	function data_state(buffer) {
		...
		if (data === InputStream.EOF) {
			...
			tokenizer._emitToken({type: 'SpaceCharacters', data: data});
		} else {
	...
	function rcdata_end_tag_name_state(buffer) {
		var appropriate = tokenizer._currentToken && (tokenizer._currentToken.name === temporaryBuffer.toLowerCase());
		var data = buffer.char();
		if (isWhitespace(data) && appropriate) {
			tokenizer._currentToken = {type: 'EndTag', name: temporaryBuffer, data: [], selfClosing: false};
			...
			tokenizer._emitToken({type: 'Characters', data: '</' + temporaryBuffer});
			buffer.unget(data);
			tokenizer.setState(rcdata_state);
		}
		return true;
	...
	function character_reference_in_attribute_value_state(buffer) {
		var character = EntityParser.consumeEntity(buffer, tokenizer, additionalAllowedCharacter);
		...
		if (additionalAllowedCharacter === '"')
			tokenizer.setState(attribute_value_double_quoted_state);
		else if (additionalAllowedCharacter === '\'')
			tokenizer.setState(attribute_value_single_quoted_state);
		else if (additionalAllowedCharacter === '>')
			tokenizer.setState(attribute_value_unquoted_state);
		return true;
	...
	try {
		while(this._state(this._inputStream));
	} catch(e) {
		if (e != InputStream.DRAIN) {
			throw(e);
		} else {
			this._inputStream.undo();
		}
	}
