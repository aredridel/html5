...
function TreeBuilder() {
	...
	this.setInsertionMode = function(name) {
		this.insertionMode = modes[name];
	...
	modes.initial.processDoctype = function(name, publicId, systemId, forceQuirks) {
		...
		if (forceQuirks || name != 'html' || (publicId != null && ([
			...
			if ((publicId == "-//W3C//DTD HTML 4.0//EN" && (systemId == null || systemId == "http://www.w3.org/TR/REC-html40/strict.dtd"))
				...
				// warning
				tree.parseError("obsolete-doctype", null, true);
	...
	modes.initial.processSpaceCharacters = function(data) {
	};

	modes.initial.processCharacters = function(data) {
	...
	modes.afterAfterBody.processSpaceCharacters = function(data) {
		modes.inBody.processSpaceCharacters(data);
	};
	...
	modes.afterFrameset.processCharacters = function(data) {
		tree.parseError("unexpected-char-after-frameset");
	};
	...
	modes.afterHead.processCharacters = function(data) {
		this.anythingElse();
		tree.insertionMode.processCharacters(data);
	};
	...
	modes.beforeHead.processCharacters = function(data) {
		this.startTagHead('head', {});
		tree.insertionMode.processCharacters(data);
	};

	modes.beforeHead.processSpaceCharacters = function(data) {
	};
	...
	modes.beforeHTML.processSpaceCharacters = function(data) {
	};

	modes.beforeHTML.processCharacters = function(data) {
		this.anythingElse();
		tree.insertionMode.processCharacters(data);
	};
	...
	modes.inColumnGroup.processCharacters = function(data) {
		var ignoreEndTag = this.ignoreEndTagColgroup();
		this.endTagColgroup('colgroup');
		if (!ignoreEndTag) tree.insertionMode.processCharacters(data);
	};
	...
	modes.inForeignContent.processCharacters = function(characters) {
	...
	modes.inHeadNoscript.processCharacters = function(data) {
		// FIXME error message
	...
	modes.inHeadNoscript.processSpaceCharacters = function(data) {
		modes.inHead.processSpaceCharacters(data);
	};
	...
	modes.inHead.processCharacters = function(data) {
		var name = tree.currentStackItem().localName;
	...
	modes.inTable.processSpaceCharacters =  function(data) {
		if (tree.currentStackItem().isFosterParenting()) {
			...
			tree.insertionMode.processSpaceCharacters(data);
		} else {
	...
	modes.inTableText.flushCharacters = function() {
		...
		if (!isAllWhitespace(characters)) {
			tree.redirectAttachToFosterParent = true;
			modes.inBody.processCharacters(characters);
			tree.redirectAttachToFosterParent = false;
		} else {
			tree.insertText(characters);
		}
	...
	modes.inTableText.processCharacters = function(data) {
		...
		if (!data)
			return;
		tree.pendingTableCharacters.push(data);
	};

	modes.inTableText.processSpaceCharacters = function(data) {
	...
	modes.inTableBody.processSpaceCharacters = function(data) {
		modes.inTable.processSpaceCharacters(data);
	};
	...
	modes.inSelect.processCharacters = function(data) {
		data = data.replace(/\u0000/g, function(match, index){
			// @todo position
			tree.parseError("invalid-codepoint");
			return '';
	...
	modes.inRow.processSpaceCharacters = function(data) {
		modes.inTable.processSpaceCharacters(data);
	};
	...
	modes.afterAfterFrameset.processSpaceCharacters = function(data) {
		modes.inBody.processSpaceCharacters(data);
	};

	modes.afterAfterFrameset.processCharacters = function(data) {
	...
	modes.inBody.processSpaceCharactersDropNewline = function(data) {
	...
	modes.inBody.processSpaceCharacters = function(data) {
		if (this.dropNewline) {
	...
	modes.inBody.processSpaceCharactersNonPre = function(data) {
		tree.reconstructActiveFormattingElements();
		tree.insertText(data);
	};

	modes.inBody.processCharacters = function(characters) {
	...
	modes.inBody.startTagA = function(name, attributes) {
		var formattingElement = tree.elementInActiveFormattingElements('a');
		if (formattingElement) {
			...
			var pos = tree.activeFormattingElements.indexOf(formattingElement);
			if (pos != -1)
				tree.activeFormattingElements.splice(pos, 1);
		}
		tree.reconstructActiveFormattingElements();
	...
	modes.text.processCharacters = function(data) {
		tree.insertText(data);
	};
...
TreeBuilder.prototype.adoptionAgencyEndTag = function(name) {
	...
	while (outerLoopCounter++ < outerIterationLimit) {
		...
		if (!this.openElements.contains(formattingElement)) {
			...
			this.activeFormattingElements.splice(this.activeFormattingElements.indexOf(formattingElement), 1);
			return true;
		}
		...
		while (innerLoopCounter++ < innerIterationLimit) {
			...
			this.activeFormattingElements[this.activeFormattingElements.indexOf(node)] = clone;
...
TreeBuilder.prototype.processToken = function(token) {
	...
	case 'Comment':
		insertionMode[method](token.data);
		break;
	case 'StartTag':
		insertionMode[method](token.name, token.data, token.selfClosing);
		break;
	case 'EndTag':
		insertionMode[method](token.name);
		break;
	case 'Doctype':
		insertionMode[method](token.name, token.publicId, token.systemId, token.forceQuirks);
		break;
	case 'EOF':
		insertionMode[method]();
		break;
	}
};
...
TreeBuilder.prototype.insertFormattingElement = function(name, attributes) {
	...
	this.activeFormattingElements.push(this.currentStackItem());
};
