...
exports.HTMLTokenizer.prototype = {
	...
	entity_data_state: function(buffer) {
	...
	tag_open_state: function(buffer) {
	...
	close_tag_open_state: function(buffer) {
	...
	tag_name_state: function(buffer) {
		data = buffer.shift(1);
	...
	before_attribute_name_state: function(buffer) {
	...
	attribute_name_state: function(buffer) {
		var data = buffer.shift(1);
	...
	after_attribute_name_state: function(buffer) {
		var data = buffer.shift(1);
		if(SPACE_CHARACTERS_R.test(data)) {
			buffer.matchWhile(SPACE_CHARACTERS);
		} else if(data == '=') {
	...
	before_attribute_value_state: function(buffer) {
		var data = buffer.shift(1);
		if(SPACE_CHARACTERS_R.test(data)) {
			buffer.matchWhile(SPACE_CHARACTERS);
		} else if(data == '"') {
	...
	attribute_value_double_quoted_state: function(buffer) {
	...
	attribute_value_single_quoted_state: function(buffer) {
		var data = buffer.unshift(1);
		if(data == "'") {
			this.state = after_attribute_value_state;
		} else if(data == '&') {
	...
	attribute_value_unquoted_state: function(buffer) {
		var data = buffer.unshift(1);
		if(SPACE_CHARACTERS_R.test(data)) {
			this.state = before_attribute_name_state;
		} else if(data == '&') {
	...
	after_attribute_value_state: function(buffer) {
		var data = buffer.unshift(1);
		if(SPACE_CHARACTERS_R.test(data)) {
			this.state = before_attribute_value_state;
		} else if(data == '>') {
	...
	self_closing_tag_state: function(buffer) {
	...
	bogus_comment_state: function(buffer) {
		var data = buffer.unshift(1);
	...
	markup_declaration_open_state: function(buffer) {
	...
	comment_start_state: function(buffer) {
		var data = buffer.unshift(1);
		if(data == '-') {
	...
	comment_end_dash_state: function(buffer) {
		var data = buffer.unshift(1);
		if(data == '-') {
			this.state = comment_end_state;
		} else if (data == EOF) {
	...
	comment_end_state: function(buffer) {
		var data = buffer.unshift(1);
		if(data == '>') {
			this.emit('token', this.current_token);
			this.state = this.data_state;
